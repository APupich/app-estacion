...
HTML;
		...
		function register($form){
			...
			if(count($result)==0){
				...
				$ssql = "INSERT INTO appestacion__users (token,email, password, token_action,add_date) VALUES ('$token','$email','$pass', '$token_email',NOW())";
...
<body style="font-family: Arial, sans-serif;">
    ...
    <div style="max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ccc; border-radius: 8px;">
        ...
        <p style="text-align: center;">
            <a href="https://mattprofe.com.ar/alumno/6826/app-estacion/verify?token='.$token_email.'" style="display: inline-block; padding: 12px 24px; background-color: #007bff; color: #fff; text-decoration: none; border-radius: 5px;">Click aquí para activar tu usuario</a>
        </p>
...
';
		...
		function update($form){
			...
			$ssql = "UPDATE appestacion__users SET active=1,token_action='',active_date=NOW() WHERE token_action='$token'";
		...
		function update_block($form){
			...
			$ssql = "UPDATE appestacion__users SET blocked=1,token_action='',blocked_date=NOW() WHERE token_action='$token'";
		...
		function new_token($email){
			...
			$ssql = "UPDATE appestacion__users SET token_action='$token' WHERE email='$email'";
		...
		function recovery($form){
			...
			$ssql = "SELECT * FROM appestacion__users WHERE email='$email'";
			$result = $this->query($ssql);
			if ($result == null) {
				return ["error" => "Email no registrado", "errno" => 404];
			}
...
<body>
    <div class="container">
        ...
        <a href="https://mattprofe.com.ar/alumno/6826/app-estacion/reset?token='.$new.'" class="button">Click aquí para restablecer contraseña</a>
    </div>
</body>
</html>
';
			...
			$ssql = "UPDATE appestacion__users SET recovery=1,recover_date=NOW() WHERE email='$email'";
			$result = $this->query($ssql);
		...
		function reset($form){
			...
			$ssql = "UPDATE appestacion__users SET recovery=0,blocked=0,recover_date=NOW(),password='$pass' WHERE token_action='$token'";
			$result = $this->query($ssql);
			...
			$new = $this->new_token($email);
			// URL para bloquear la cuenta
			$blockUrl = 'https://mattprofe.com.ar/alumno/6826/app-estacion/blocked?token='.$new;
$cuerpo_email = <<<HTML
<!DOCTYPE html>
<html lang="es">
<head>
